...
# TFC Agent version
TFC_AGENT_VERSION="1.10.0"

# Get agent archive
curl -o agent.zip --location "https://releases.hashicorp.com/tfc-agent/${TFC_AGENT_VERSION}/tfc-agent_${TFC_AGENT_VERSION}_linux_amd64.zip"

# Download the ignature files.
curl -Os https://releases.hashicorp.com/tfc-agent/${TFC_AGENT_VERSION}/tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS
curl -Os https://releases.hashicorp.com/tfc-agent/${TFC_AGENT_VERSION}/tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS.sig
...
gpg --verify tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS.sig tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS

# Verify the SHASUM matches the archive.
shasum -a 256 -c tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS

mkdir /agent
unzip agent.zip -d /agent
rm tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS
rm tfc-agent_${TFC_AGENT_VERSION}_SHA256SUMS.sig
rm -f agent.zip
